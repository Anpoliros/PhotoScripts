...
class ScriptExecutor {
    static func executeJavaScript(
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            // Construct Java command using ConfigLoader paths
            let classPath = ConfigLoader.getCompiledClassesDirectory()
            let scriptsPath = ConfigLoader.getScriptsDirectory()

            // Determine source file path
            let srcPath: String
            if FileManager.default.fileExists(atPath: script.scriptPath) {
                // Use absolute path from script
                srcPath = (script.scriptPath as NSString).deletingLastPathComponent
            } else {
